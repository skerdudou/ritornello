...
describe('i18n keys used by the shell', () => {
  it('all exist in the embedded English catalog (core + common)', () => {
    ...
    const used = new Set([
      ...
      // InstallablesDialog.vue: a component kind is rendered through
      // `` t(`plugin_kind_${k}`) ``, a template literal — the regex's char
      // class is `['"]` and never matches a backtick, so these four keys
      // (the closed vocabulary `ComponentOffer`'s catalogue entries carry)
      // are otherwise invisible to it. Without this explicit addition, a
      // symmetric deletion of all four keys from both packs would pass this
      // guard silently, exactly the failure mode it exists to catch.
