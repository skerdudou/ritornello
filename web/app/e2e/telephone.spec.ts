...
test('sur téléphone : glisser le curseur de volume envoie un SetVolume que le cœur renvoie', async ({ page }) => {
  ...
  // local), la trame SSE ne l'est pas : sans attendre la reponse du POST, la
  ...
  // `cmd_tx` et repond aussitot, avant que la boucle du coeur ne traite la
  // commande (appel mpv puis publication sur le canal d'etat). Une seule
  ...
  // reçoit l'etat courant des l'ouverture, voir parcours.spec.ts).
  const lireVolumeSse = () =>
    page.evaluate(
      () =>
