//! Evenements du player mpv : fin de playback, restart a rebours croissant, reprise au reveil, et la demande relayee a la source active.
...
impl<P: Player> Core<P> {
    ...
    pub async fn handle_event(&mut self, ev: Event) -> EventOutcome {
        match ev {
            ...
            // du stream : la boucle de `main` (qui tient l'échéance `retry_at`)
            ...
            // Volontairement sans effet sur `retry_count` : la vivacité du stream
            // est déjà attestée par `PlaybackActive`, et un titre ICY n'est pas
            // une preuve de playback (une station peut en send_frame un puis se
            ...
            // métadonnées ne prouvent pas que la playback est vivante.
            Event::FileTags(track) => self.handle_file_tags(*track),
            // Même statut que les tags vis-à-vis de `retry_count` : le path
            // n'atteste rien de la vivacité du stream, il sert uniquement à la
            // cover embarquée.
            Event::Path(path) => self.handle_path(path),
            // Le player a changé de piste de lui-même : fin de piste d'un
            ...
            Event::PlaybackIdle => {
                ...
                // Eof de playback **normale** (fin de disque, notamment) : le
                // dire à la Source, seule à pouvoir recaler son état de
                // playback, sa vue et son identité — le cœur ne peut pas
                // inventer « plus rien ne plays » à sa place, l'identité est
                ...
                //
                // Plus rien ne plays : sans cela, les tags du dernier fichier
    ...
    /// fenêtre de rendez-vous s'announcement à t+30 s et est câblé sans redémarrage,
    ...
    /// première source triée, donc le name n'est introuvable que si la table est
    /// **clear** — et il aurait échangé un refus de démarrer lisible contre un
    /// arrêt brutal au démarrage, sans page pour le raconter.
    ///
    /// Sans source, une commande **ne fait rien** et le dit en `debug` : ce
    /// n'est pas une anomalie, seulement un appareil qui n'a rien à read.
    /// Un `warn` remplirait le buffer d'erreurs de l'IHM à chaque touche.
...
mod tests {
    ...
    async fn resume_sans_aucune_source_publie_letat_au_lieu_de_paniquer() {
        ...
        // premier à mourir : `active()` paniquait sur une table clear, et `resume`
        ...
        // voulait y voir les plugins figés.
        let (mut core, mut state_rx, dir) = setup_without_source();
        core.resume().await.unwrap();
        let state = state_rx.borrow_and_update().clone();
        assert_eq!(state.source, "", "la chaine clear EST l'absence, c'est au rendition de la nommer");
        assert!(!state.standby, "le coeur demarre, il n'entre pas en veille pour autant");
        drop(dir);
    }
    ...
    async fn backoff_croissant_puis_reinitialise_par_un_titre() {
        ...
        assert!(d2 > d1);
        // Un titre atteste la vivacité du stream : c'est aussi le verdict que la
        // boucle de `main` suit pour annuler l'échéance de restart.
    ...
    async fn wake_noop_ne_declenche_pas_de_retry_cd_reste_silencieux() {
        // Regression (revue finale 2.2) : le cd repond Noop a Wake (pas de playback
        // au boot/reveil). L'ancienne porte de retry (!stopped) laissait quand meme
        // planifier une restart sur le prochain PlaybackIdle, ce qui faisait
    ...
    async fn un_contenu_fini_n_arme_pas_la_relance_un_flux_live_si() {
        ...
        // `idle` exactement comme lors d'une coupure de stream. Tant que le cœur
        // reniflait l'URI (`cdda://`), un path de fichier tombait du mauvais
        // côté — restart exponentielle au lieu de l'arrêt propre, et la liste
        // repartait en boucle depuis la première piste.
        let (mut core, _pc, _sc, _rx, _d) = setup();
        core.apply(SourceAction::play("/var/lib/ritornello/plugin-files.m3u").finite())
            .await
            .unwrap();
        assert!(!core.expecting_stream, "un contenu fini ne doit pas armer la restart");

        core.apply(SourceAction::play("http://icecast/fip.mp3")).await.unwrap();
        assert!(core.expecting_stream, "un stream live doit rester relançable");
    }
    ...
    async fn une_liste_se_charge_par_load_list_puis_se_positionne() {
        ...
        // bornes, la playback repartait de la première piste, et l'affichage
    ...
    async fn un_media_reste_charge_par_loadfile() {
        ...
        // un `.m3u8` est une liste pour un player de fichiers et un stream HLS
    ...
    async fn un_play_sans_index_ne_positionne_rien() {
        // Le path de la radio : aucune commande superflue sur la socket mpv.
        let (mut core, player_calls, _sc, _rx, _d) = setup();
        core.apply(SourceAction::play("http://icecast/fip.mp3")).await.unwrap();
    ...
    async fn wake_play_declenche_bien_un_retry_apres_idle() {
        ...
        // un stream est bien attendu, donc un PlaybackIdle doit programmer un retry.
        let (mut core, _pc, _sc, _rx, _d) = setup();
        core.resume().await.unwrap();
    ...
    async fn la_fin_du_disque_ne_relance_pas_la_lecture_et_previent_la_source() {
        ...
        // machinerie de restart des stream réseau : `Activate` → `Play cdda://`
        ...
        // Eof du disque : pas de restart, et la Source est prévenue — elle
        // seule peut recaler sa vue et son identité sur « plus rien ne plays ».
        assert_eq!(core.handle_event(Event::PlaybackIdle).await, EventOutcome::Nothing);
