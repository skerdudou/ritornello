//! Correspondance entre l'URL d'un flux et l'identifiant de station.
//!
...
//! (qui contient `id_station=<n>`). `scripts/fetch-stations.mjs` la régénère.
//!
...
//! reconnaître ses stations, et l'échec d'une telle lecture serait silencieux.
...
const EMBARQUEE: &str = include_str!("stations.toml");
...
pub struct Station {
    ...
    /// configurée dans `stations.toml` (voir `contient_jeton`).
    ///
    ...
    /// nom historique `direct.fipradio.fr/live/<mount>-midfi.mp3` (qui redirige
    ...
    pub id: u32,
    /// Profil de rendu à demander pour cette station (dernier segment de
    /// l'URL du direct, voir `live::url_direct`).
    ///
    ...
    #[serde(default = "profil_par_defaut")]
    pub rules: String,
}

/// Profil retenu quand une entrée n'en déclare pas.
fn profil_par_defaut() -> String {
...
pub fn contient_jeton(url: &str, mount: &str) -> bool {
    if mount.is_empty() {
        return false;
    }
    let octets = url.as_bytes();
    url.match_indices(mount).any(|(debut, _)| {
        let fin = debut + mount.len();
        let avant_libre = debut == 0 || !octets[debut - 1].is_ascii_alphanumeric();
        let apres_libre = fin >= octets.len() || !octets[fin].is_ascii_alphanumeric();
...
impl Table {
    ...
    pub fn load(path: &Path) -> Self {
        ...
        stations.extend(Self::embarquee().stations);
        Self { stations }
    }
    ...
    pub fn embarquee() -> Self {
        toml::from_str(EMBARQUEE).expect("table de stations embarquee valide")
    }

    /// Station correspondant à cette URL de flux, s'il y en a une. Premier
    /// accord, dans l'ordre de la table.
    pub fn station_pour(&self, url: &str) -> Option<&Station> {
        self.stations.iter().find(|s| s.mounts.iter().any(|m| contient_jeton(url, m)))
    }
}
...
mod tests {
    ...
    fn le_jeton_respecte_ses_bords() {
        assert!(contient_jeton("https://icecast.radiofrance.fr/fip-midfi.mp3", "fip"));
        assert!(contient_jeton("https://stream.radiofrance.fr/fip/fip.m3u8", "fip"));
        assert!(contient_jeton("https://stream.radiofrance.fr/fip/fip_midfi.m3u8", "fip"));
        assert!(contient_jeton("https://direct.fipradio.fr/live/fip-midfi.mp3", "fip"));
        // Le cœur du problème : un préfixe ne doit pas capturer les autres.
        assert!(!contient_jeton("https://icecast.radiofrance.fr/fipgroove-midfi.mp3", "fip"));
        assert!(!contient_jeton("https://direct.fipradio.fr/live/fipgroove-midfi.mp3", "fip"));
        assert!(!contient_jeton("https://icecast.radiofrance.fr/francemusiquebaroque-midfi.mp3", "francemusique"));
        // Un mount vide ne correspond à rien (sans quoi il correspondrait à tout).
        assert!(!contient_jeton("https://icecast.radiofrance.fr/fip-midfi.mp3", ""));
    }
    ...
    fn un_caractere_non_ascii_fait_bord() {
        ...
        assert!(contient_jeton("https://exemple.test/é/fip/é", "fip"));
    }
    ...
    fn chaque_station_porte_un_profil_connu() {
        ...
        for (id, attendu) in [
            ...
            // Mouv' et les locales : mesuré, seul ce profil sort le morceau.
            (6, "webrf_mouv_player"),
    ...
    fn la_table_embarquee_est_valide_et_complete() {
        ...
        for s in &t.stations {
            ...
            assert!(s.mounts.iter().all(|m| !m.is_empty()), "{}: mount vide", s.label);
    ...
    fn reconnait_les_trois_formes_durl_dune_meme_station() {
        ...
        // ou d'un lecteur : seul le mount est commun aux trois.
        let t = Table::embarquee();
        for url in [
            ...
            assert_eq!(t.station_pour(url).map(|s| s.id), Some(66), "{url}");
        }
    }
    ...
    fn les_grandes_stations_et_les_locales_sont_reconnues() {
        ...
        for (url, attendu) in [
            ...
            // Locale dont le mount ne ressemble pas à son nom d'antenne.
            ...
            let s = t.station_pour(url).unwrap_or_else(|| panic!("{url} non reconnue"));
    ...
    fn une_url_inconnue_ne_correspond_a_rien() {
        ...
        let t = Table::embarquee();
        assert!(t.station_pour("https://ouifm3.ice.infomaniak.ch/ouifm3.mp3").is_none());
        assert!(t.station_pour("https://somafm.com/groovesalad256.pls").is_none());
        // Le site de Radio France n'est pas un flux, mais il porte les mêmes
        ...
        assert!(t.station_pour("https://www.radiofrance.fr/").is_none());
    }
    ...
    fn le_fichier_de_loperateur_est_consulte_avant_la_table_embarquee() {
        ...
        assert_eq!(t.station_pour(url).map(|s| s.id), Some(999), "correction");
        assert_eq!(t.station_pour("https://x/nouveauflux.mp3").map(|s| s.id), Some(123), "ajout");
        // Le reste de la table embarquée continue de répondre.
        assert_eq!(
            t.station_pour("https://icecast.radiofrance.fr/fipreggae-midfi.mp3").map(|s| s.id),
            Some(71),
    ...
    fn fichier_absent_laisse_la_table_embarquee_intacte() {
        ...
        assert_eq!(t.stations.len(), Table::embarquee().stations.len());
    }
    ...
    fn fichier_invalide_laisse_la_table_embarquee_intacte() {
        ...
        assert_eq!(Table::load(&p).stations.len(), Table::embarquee().stations.len());
    }

    #[test]
    fn un_mount_vide_ne_correspond_pas_a_tout() {
        // Sans la garde de `contient_jeton`, une entrée mal renseignée ferait
        // interroger cette station pour **toutes** les URL.
        let t: Table = toml::from_str("[[station]]\nmounts = [\"\"]\nid = 1\n").unwrap();
        assert!(t.station_pour("https://icecast.radiofrance.fr/fip-midfi.mp3").is_none());
        let vide: Table = toml::from_str("[[station]]\nmounts = []\nid = 1\n").unwrap();
        assert!(vide.station_pour("https://x/y").is_none());
    }
