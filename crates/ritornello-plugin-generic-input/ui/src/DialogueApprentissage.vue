 ...
 * vérité (l'état `ajouter`, le sondage, l'annulation côté plugin).
 */
const props = defineProps<{
  ...
  /** Libellé **déjà traduit** de l'action apprise, pour le titre. */
  action: string
 ...
 * garde, le titre afficherait « Apprentissage d'une touche — » pendant toute
 * la fermeture.
 */
    ...
    <DialogContent data-dlg-learn>
      ...
      <label class="flex items-center gap-2 text-sm">
        <input
          type="checkbox"
          data-learn-append
          :checked="props.ajouter"
          @change="emit('update:ajouter', ($event.target as HTMLInputElement).checked)"
        />
