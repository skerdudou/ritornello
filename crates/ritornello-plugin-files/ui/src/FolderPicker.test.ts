// `FolderPicker` est une **leaf** entièrement pilotée par ses propriétés :
// il ne parle ni au server ni à une popin. Il se monte donc directement, sans
// le harness de page — le faire passer par `FilesAdmin` ferait dépendre ces
...
function mountPicker(surcharges: Partial<Exploration> = {}, path = '/media/usb') {
  return mount(FolderPicker, {
    props: { exploration: { ...BASE, ...surcharges }, t, fige: false, path },
  })
}
...
describe('FolderPicker', () => {
  ...
  it('descend émet le nom du dossier, pas un path', async () => {
    // C'est l'appelant qui sait composer le path : un path local et un
    // path SMB ne se composent pas de la même façon.
    const w = mountPicker()
  ...
  it('pendant une attente rien n’est cliquable et l’attente se voit', () => {
    // Sans le disabled, un double clic impatient empilerait deux journey ;
    ...
    expect(w.get('[data-choix-goUp]').attributes('disabled')).toBeDefined()
  })
  ...
  it('un refus s’affiche à la place de l’arbre', () => {
    ...
    const w = mountPicker({ error: 'hôte injoignable' })
  ...
  it('affiche le path qu’on lui donne, pas celui de l’exploration', () => {
    // Correctif d'un défaut signalé : côté partage, `exploration.path` est
    // relatif au partage, si bien que le partage « disparaissait » du path dès
    ...
    expect(w.get('[data-choix-path]').text()).toBe('//192.168.1.15/music/Yann Tiersen')
  })

  it('truncated un path trop long par le début et garde le tout en infobulle', () => {
    ...
    const span = w.get('[data-choix-path]')
