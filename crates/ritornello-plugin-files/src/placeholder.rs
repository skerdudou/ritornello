// ESM module served as long as the plugin UI has not been built.
//
// Included **textually** by `build.rs` (`include!`) as well as compiled as a
// module of the crate: this is what allows testing the making of the
// placeholder with `cargo test`, whereas Cargo never runs the tests of a build
// script. No external dependency allowed here.
//
// Note: this module comment is deliberately an ordinary comment (`//`) and not
// an inner doc (`//!`). An inner doc triggers `E0753 : expected outer doc
// comment` once this file is included as is by `build.rs` via `include!` — the
// compiler's restriction bears on the position in the token stream of the host
// file, not on the source file as read here (see
...
/// distinguish an already present placeholder asset from a real deliverable.
pub const MARKER: &str = "ritornello-ihm-plugin-non-construite";

/// Deliberately invalid contract: the shell then shows its "plugin to rebuild"
/// message, which describes the situation exactly.
...
/// left behind a rebuilt `ui.js` would give a UI without any style, another
/// silent degradation.
...
/// target, but `ui/dist/ui.js` now exists — said nothing more, and the release
/// binary embedded the placeholder in silence.
...
mod tests {
    ...
    fn is_placeholder_does_not_trigger_on_a_real_deliverable() {
        ...
        let real_js = "import{defineComponent as e}from\"view\";\
