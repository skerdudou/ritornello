...
//!
//! **En-tête seulement, dans le processus** — pas de `ffprobe` ni de mpv.
...
/// Durée du fichier en secondes, ou `None` si on ne sait pas la lire.
///
...
/// « 0:00 » affirmerait une piste vide là où le tiret dit qu'on ne sait pas.
pub fn sonder(chemin: &Path) -> Option<u32> {
...
mod tests {
    ...
    ///
    /// Pas de fichier binaire versionné dans le dépôt : la durée dépend de
    ...
    fn la_duree_dun_mp3_se_lit_dans_son_en_tete() {
        ...
        let d = sonder(&f).expect("une duree attendue");
    ...
    fn un_fichier_illisible_ne_fait_pas_echouer_le_sondage() {
        ...
        assert_eq!(sonder(&f), None);
        assert_eq!(sonder(&dir.path().join("absent.mp3")), None);
    }

    #[test]
    fn une_duree_nulle_est_rendue_inconnue() {
        // « 0:00 » affirmerait une piste vide ; `None` fait afficher un tiret,
        // qui dit qu'on ne sait pas. La page s'appuie déjà sur cette
        // distinction (voir `formaterDuree`).
        let dir = tempfile::tempdir().unwrap();
        let f = dir.path().join("vide.flac");
