// Safety net, modelled on the mpd plugin's: every key called by this plugin's
// source code must exist in
// `crates/ritornello-plugin-musicbrainz/src/locales/en.toml` + the common
// vocabulary `crates/ritornello-i18n/src/locales/common_en.toml` (the plugin
// consumes both, the common layer being merged by `Catalog::entries`).
// Without that guarantee, a missing key is displayed as is on screen instead
// of failing at build or test time.
...
// a literal key. This plugin has no dynamically built key (the origin keys in
// `originText` are string literals passed straight to `t.value(...)`, not
// `t(something + suffix)`): the regex alone is enough, as for the mpd plugin.
...
describe('i18n keys used by the musicbrainz plugin', () => {
