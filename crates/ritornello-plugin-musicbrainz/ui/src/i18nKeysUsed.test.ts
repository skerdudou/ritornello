// Garde-fou, sur le modele de celui du greffon mpd : toute cle appelee par le
// code source de ce plugin doit exister dans
// `crates/ritornello-plugin-musicbrainz/src/locales/en.toml` + le vocabulaire
...
// `Catalog::entries`). Sans cette garantie, une cle absente s'affiche telle
// quelle a l'ecran au lieu d'echouer au build ou aux tests.
...
// une cle litterale. Ce plugin n'a aucune cle construite dynamiquement (les
// cles de `CLES_ORIGINE` sont des litteraux de chaine passes ensuite a
// `t.value(...)`, pas des `t(quelqueChose + suffixe)`) : la regex suffit
// seule, comme pour le greffon mpd.
...
describe('cles i18n utilisees par le plugin musicbrainz', () => {
