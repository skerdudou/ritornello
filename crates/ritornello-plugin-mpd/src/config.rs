...
impl Config {
    /// Charge la config depuis `chemin`, ou retombe sur les défauts en
...
mod tests {
    ...
    fn valider_rend_des_cles_de_catalogue_et_non_des_phrases() {
        ...
        let vide = Config { listen: String::new(), port: 6600 };
        assert_eq!(vide.valider().unwrap_err(), "listen_empty");
        let zero = Config { listen: "0.0.0.0".into(), port: 0 };
        assert_eq!(zero.valider().unwrap_err(), "port_zero");
    }
    ...
    fn lenregistrement_refuse_une_config_invalide_sans_toucher_au_disque() {
        ...
        assert_eq!(invalide.enregistrer(&chemin).unwrap_err(), "port_zero");
        assert!(!chemin.exists(), "rien ne doit etre ecrit quand la validation refuse");
    }
    ...
    fn une_config_syntaxiquement_valide_mais_refusee_retombe_aussi_sur_les_defauts() {
        // Distinct du test precedent : ici le toml parse, mais `valider()`
        // refuse le contenu (port a 0). `charger` doit retomber sur les
        // defauts dans ce cas aussi, pas seulement sur une erreur de parsing.
        let dir = tempfile::tempdir().unwrap();
        let chemin = dir.path().join("mpd.toml");
        std::fs::write(&chemin, "port = 0\n").unwrap();
        assert_eq!(Config::charger(&chemin), Config::default());
    }
}
