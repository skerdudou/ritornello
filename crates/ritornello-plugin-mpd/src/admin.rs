...
/// pour `Config::load`, qui complète un *fichier* partiel — mais faux pour
/// une écriture, où un `PUT {"port": 6601}` sans `listen` doit être refusé,
/// pas silencieusement compris comme « put_back `listen` à `0.0.0.0` ».
/// Même séparation playback/écriture que `generic-input::admin::Op::Save` et
...
pub struct MpdAdmin {
    ...
    /// alors sans redémarrage (voir `session::listen`).
    ///
...
impl AdminPlugin for MpdAdmin {
    ...
    async fn set_data(&mut self, data: serde_json::Value) -> Result<(), String> {
        // `ConfigWrite`, pas `Config` : voir le commentaire sur le type,
        ...
        // `save` valide puis écrit atomiquement ; dans les deux cas
        // d'échec, elle renvoie une **clé** de sources_catalog (`listen_empty`,
...
mod tests {
    ...
    async fn un_enregistrement_reussi_fait_relier_la_moitie_reseau() {
        ...
        // main. La page écrit le fichier **puis** push_cover la configuration ; la
        ...
        // Et ce qui est poussé est bien ce qui est sur disque : serve une
    ...
    async fn un_enregistrement_refuse_ne_fait_rien_relier() {
        ...
        assert!(!rx.has_changed().unwrap(), "rien ne doit etre push_cover sur un refus");
    }
    ...
    async fn un_port_invalide_renvoie_une_phrase_de_catalogue_pas_la_cle_brute() {
        ...
        // `Err("port_zero".into())` sans le résoudre via le sources_catalog. La page
    ...
    async fn un_echec_decriture_renvoie_une_phrase_de_catalogue_pas_le_detail_io() {
        ...
        // `save(...).map_err(|e| e.to_string())` mettrait le détail I/O
    ...
    async fn une_requete_mal_formee_renvoie_une_erreur_traduite() {
        // `ConfigWrite` (le type du corps de `SetData`, distinct de
    ...
    async fn un_champ_manquant_est_refuse_plutot_que_complete_par_un_defaut() {
        ...
        // corrects pour *load un fichier* partiel) aurait laissé un `PUT
        // {"port": 6601}` sans `listen` réussir silencieusement, en
        // persistant `listen = "0.0.0.0"` comme si l'opérateur l'avait
        // demandé — une pushback à zéro de l'adresse d'écoute déguisée en
        // enregistrement réussi. Avec `ConfigWrite` (champs obligatoires),
