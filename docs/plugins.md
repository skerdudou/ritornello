...
four bundled plugins keep exactly the precedence they have today without
...
- `ritornello-plugin-radiofrance-metas` reads the *live* endpoint of Radio
  ...
  This is the plugin with the strongest case of the four: Radio France's
...
- `ritornello-plugin-nrj-metas` reads the on-air endpoint of the NRJ
  group's four brand sites. **Nothing to configure**: the table of the
  group's 365 stations is embedded in the binary (`src/stations.toml`) —
  191 on nrj.fr, 92 on nostalgie.fr, 57 on cheriefm.fr, 25 on
  rireetchansons.fr. No station is listed under two brands, and a host
  only serves its own stations, so the table keeps the brand next to each
  station's identifier.

  The group's own ICY is worse than useless: it carries an internal cart
  code duplicated on both sides of the separator (`DD25-19 - DD25-19` on
  Rire & Chansons, `5612 - 5612` on NRJ). There is nothing to split, and
  it *looks* like an ordinary artist/title pair, so left alone it would be
  displayed as one. Each brand site's own on-air endpoint, queried per
  station, answers with artist and title already separated — but
  `cheriefm.fr` refuses the request outright without a full browser
  `User-Agent`, measured, which is why the plugin (and the script that
  regenerates its table) send one.

  The endpoint's own deadline runs 40 to 70 seconds late, measured, so the
  plugin does not poll on it: the rhythm is driven by the ICY cart code
  the core already hands it, measured to change up to 33 seconds ahead of
  the endpoint catching up, with the announced deadline kept only as a
  safety net. A 30-second advert plays on connection, invisible to every
  `metadata` plugin (the core discards a blank `StreamTitle` as noise rather
  than reporting it), which is why the first query fires on recognizing the
  ...
  measured timings, the filler and wallpaper-cover rules, and why
  `musicbrainz`'s own ICY probe needs no special case for these stations.
